...
def predict():
    ...
    try:
        ...
        # Gradient Boosting predictions (best single model — 83.94% avg accuracy)
        p_cpu   = gb_models['cpu'].predict_proba(X)
        p_ram   = gb_models['ram'].predict_proba(X)
        p_stype = gb_models['storage_type'].predict_proba(X)
        p_ssize = gb_models['storage_size'].predict_proba(X)
        p_gpu   = gb_models['gpu'].predict_proba(X)
...
if __name__ == '__main__':
    print("PC Recommender V3.2 (GB only) running at http://localhost:5000")
