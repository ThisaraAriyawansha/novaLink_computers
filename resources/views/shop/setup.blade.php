...
.form-label { display:block; font-weight:600; font-size:14px; color:#333; margin-bottom:7px; }
.form-control { width:100%; padding:10px 14px; border:1.5px solid #ddd; border-radius:8px; font-size:14px; outline:none; transition:border-color 0.2s; box-sizing:border-box; }
.form-control:focus { border-color:#1a3a6b; }
textarea.form-control { resize:vertical; min-height:90px; }
.preview-img { max-width:120px; max-height:80px; border-radius:8px; margin-top:8px; object-fit:cover; }
.btn-save { background:#1a3a6b; color:white; padding:11px 28px; border:none; border-radius:8px; font-size:15px; font-weight:600; cursor:pointer; transition:background 0.2s; }
.btn-save:hover { background:#14306a; }
...
<div class="setup-wrap">
    @if(session('success'))
        <div class="alert alert-success alert-dismissible fade show" style="margin-bottom:20px;" role="alert">
    ...
    <div class="setup-card">
        <div class="section-title"><i class="fas fa-store" style="margin-right:8px;"></i>Shop Profile Setup</div>
        ...
        <form method="POST" action="{{ route('shop.store') }}" enctype="multipart/form-data">
            ...
            <div class="grid-2">
                <div class="form-group">
                    ...
                    @if(!empty($shop->logo))
                        ...
                        <div style="font-size:12px;color:#888;margin-top:4px;">Current logo — upload new to replace</div>
                    @endif
                </div>
                <div class="form-group">
                    ...
                    @if(!empty($shop->cover_image))
                        ...
                        <div style="font-size:12px;color:#888;margin-top:4px;">Current cover — upload new to replace</div>
                    @endif
                </div>
            </div>
            ...
            <button type="submit" class="btn-save">
                <i class="fas fa-save" style="margin-right:7px;"></i>Save Shop Profile
            </button>
        </form>
    </div>
</div>
